...
export const getCandidateInterviewsQuery = z.object({
...
}).describe("Candidate interviews query parameters");

export async function getCandidateInterviews(
  env: Env,
  query: z.infer<typeof getCandidateInterviewsQuery>
) {
  const token = await getAuthToken(env);
  const baseUrl = `https://${env.COMPANY}.${env.ENVIRONMENT}.com/api/v1`;
  const url = new URL(`${baseUrl}/hire/jobs/${query.jobId}/candidate/${query.candidateId}/interviews`);
